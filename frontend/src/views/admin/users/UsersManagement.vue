...
<script setup lang="ts">
  ...
  // ✅ New Logic: Determine when to show the Grid Skeletons
  // We show skeletons if loading initially, OR if fetching new filter data.
  // We do NOT show grid skeletons if we are just fetching the next page (infinite scroll).
  const showGridLoading = computed(() => {
    return isLoading.value || (isFetching.value && !isFetchingNextPage.value);
  });
