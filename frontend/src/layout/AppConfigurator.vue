...
function loadThemeConfig() {
    ...
    if (saved) {
        ...
        layoutConfig.primary = config.primary;
        layoutConfig.surface = config.surface;
        layoutConfig.menuMode = config.menuMode;

        // Update local refs
        preset.value = config.preset;
        menuMode.value = config.menuMode;

        // Apply the theme
        const presetValue = presets[config.preset];
        const surfacePalette = surfaces.value.find((s) => s.name === config.surface)?.palette;

        $t().preset(presetValue).preset(getPresetExt()).surfacePalette(surfacePalette).use({ useDefaultOptions: true });
    }
}

// Now it is safe to load the config
loadThemeConfig();
</script>
