<template>
    <Dialog :visible="visible" @update:visible="emit('update:visible', $event)" :style="{ width: '100%', maxWidth: '600px' }" :header="localXRay.id ? 'Edit X-Ray' : 'Add X-Ray'" modal>
        <div class="flex flex-col gap-4">
            ...
            <div>
                ...
                <div
                    ...
                    :class="{ 'border-red-500': submitted && !localXRay.imageUrl && !selectedFile && !fileIsUploading }"
                    @drop.prevent="onDrop"
                    @dragover.prevent
                >
                    <!-- Image exists -->
                    <template v-if="selectedFile || localXRay.imageUrl">
                        <div class="flex-shrink-0 w-full sm:w-28 h-36 sm:h-28 flex items-center justify-center">
                            <img :src="previewUrl || localXRay.imageUrl" alt="X-Ray" class="w-full h-full object-contain rounded" />
                        </div>

                        <!-- File info & status -->
                        <div class="flex-1 flex flex-col justify-center gap-1 min-w-0">
                            <div class="font-semibold truncate">
                                {{ selectedFile ? selectedFile.name : getFileName(localXRay.imageUrl) }}
                            </div>
                            <div class="flex flex-wrap items-center gap-2 text-sm text-surface-500">
                                <span>{{ selectedFile ? formatSize(selectedFile.size) : '' }}</span>
                                <span class="inline-block px-2 py-0.5 rounded text-xs text-white" :class="selectedFile ? 'bg-yellow-500' : 'bg-green-500'">
                    ...
                    <template v-else>
                        <div class="flex flex-col items-center justify-center w-full h-full text-center">
                            ...
                            <p>or</p>
                            <Button label="Choose X-Ray image" class="mt-2" @click="fileInput!.click()" />
                        </div>
                    </template>
                ...
                <small v-if="submitted && !localXRay.imageUrl && !selectedFile" class="text-red-500 mt-2 block"> Image is required. </small>
            </div>
        ...
        <template #footer>
            ...
            <Button label="Save" icon="pi pi-check" :loading="isProcessing || fileIsUploading" @click="onSave" />
        </template>
    </Dialog>
</template>
...
<script setup lang="ts">
    ...
    const localXRay = ref<Partial<XRay>>({});
    ...
    const isFormValid = computed(() => !!localXRay.value.imageUrl || !!selectedFile.value || fileIsUploading.value);
