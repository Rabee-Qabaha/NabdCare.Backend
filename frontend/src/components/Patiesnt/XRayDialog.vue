<template>
  ...
  >
    <div class="flex flex-col gap-4">
      ...
      <div>
        <label class="mb-2 block font-bold">Image</label>

        <!-- Drag-and-drop container -->
        <div
          class="flex min-h-[180px] w-full flex-col gap-4 rounded-lg border-2 border-dashed p-4 sm:flex-row sm:items-center sm:justify-start"
          :class="{
            'border-red-500': submitted && !localXRay.imageUrl && !selectedFile && !fileIsUploading,
          }"
          @drop.prevent="onDrop"
          @dragover.prevent
        >
          ...
          <template v-if="selectedFile || localXRay.imageUrl">
            <div class="flex h-36 w-full flex-shrink-0 items-center justify-center sm:h-28 sm:w-28">
              <img
          ...
          <template v-else>
            <div class="flex h-full w-full flex-col items-center justify-center text-center">
              <i class="pi pi-cloud-upload mb-2 text-4xl text-surface-400"></i>
