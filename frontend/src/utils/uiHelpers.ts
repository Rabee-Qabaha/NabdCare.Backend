...
const DEFAULT_TAG_CLASSES = 'px-2 py-1 rounded text-xs font-semibold border-0 inline-flex items-center';
...
export function getPaymentRenderInfo(method: PaymentMethod | string) {
    const config = paymentConfig[method as PaymentMethod] ?? { icon: 'pi pi-fw pi-receipt', style: 'bg-gray-300 text-gray-800' };
    return {
        ...config,
