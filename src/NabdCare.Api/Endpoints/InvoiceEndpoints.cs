...
public static class InvoiceEndpoints
{
    public static void MapInvoiceEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        // ============================================
        group.MapPost("/{id:guid}/write-off", async (
                Guid id,
                [FromBody] WriteOffRequestDto request,
                [FromServices] IInvoiceService service) =>
            {
                var invoice = await service.WriteOffInvoiceAsync(id, request.Reason);
                return invoice != null ? Results.Ok(invoice) : Results.NotFound();
            })
            .RequireAuthorization()
            .RequirePermission(Permissions.Invoices.WriteOff)
            .WithName("WriteOffInvoice")
            .WithSummary("Mark an invoice as Uncollectible (Bad Debt)")
            .Produces<InvoiceDto>()
